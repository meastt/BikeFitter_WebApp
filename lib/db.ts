...
export async function createFit(
  ...
  // First verify the bike belongs to the user
  const bike = await getBike(bikeId, userId)
  if (!bike) throw new Error('Bike not found or unauthorized')
...
export async function getLatestFit(bikeId: string, userId: string) {
  const supabase = await createClient()
